...
E <- function(stalk) { # im excluding s for now bc idk if it should be here
  # s = expected location, where coordinates = y + displacement and x+ displacement from the equaiton, 
  s <- list("s_x", "s_y")
    for (i in 1:length(stalk[[1]])) {
    if (stalk[[1]][[i]] == "x"){ #general: if i = x we want to assocate value of x .
      # wanted to have program figure out variables for us so we dont have to define by ourselves. 
      ...
      t <- stalk[[2]][[i]]
    } else if (stalk[[1]][[i]] == "s"){ # figure out if this should be here. 
      s <- stalk[[2]][[i]] # make s a variable that is a two dim vector. 
  }
...
entitytest[1][[1]]=="x"|| entitytest[[1]]=="y"|| entitytest[[1]]=="z"||
      entitytest[[1]]=="v_x" || entitytest[[1]] == "v_y"
...
U4_pr1 <- function(stalk){
  talk[[1]][stalk[1][[1]]=="Theta2"]
}

#entitytest[[1]][entitytest[1][[1]]=="x"| entitytest[[1]]=="y"| entitytest[[1]]=="z"|
#                        entitytest[[1]]=="v_x" | entitytest[[1]] == "v_y"]
```
