...
FinSheaf %>%
  ...
  unnest(stalkoutput) %>%
  ungroup() %>%
  mutate(ConsRad = summarise(across(5:14, ~ sd(.,na.rm = TRUE)))) # I summarized bc I just wanted 1, gives new DF though. 

FinSheaf %>%
  group_by(SDest) %>%
  unnest(stalkoutput) %>%
  ungroup() %>%
  mutate(sd = across(5:14, ~ sd(.,na.rm = TRUE)))%>%
  mutate(ConsRad = )
...
mtcars %>%
  mutate(sd = across(.cols = mpg:gear, .fns = ~sd(.))) -> looktest
